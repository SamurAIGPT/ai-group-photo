...
export async function GET(req) {
  try {
    ...
    if (id) {
      const creation = await prisma.groupPhotoCreation.findUnique({
        ...
        try {
          ...
          if (checkRes.ok) {
            ...
            if (state === "completed" || state === "succeeded") {
              ...
              if (imageUrl) {
                const updated = await prisma.groupPhotoCreation.update({
    ...
    const creations = await prisma.groupPhotoCreation.findMany({
        ...
        try {
          ...
          if (checkRes.ok) {
            ...
            if (state === "completed" || state === "succeeded") {
              ...
              const imageUrl = outputs[0] || (checkJson.output ? checkJson.output[0] : "");
              if (imageUrl) {
                return prisma.groupPhotoCreation.update({
                  where: { id: creation.id },
                  data: {
                    status: "completed",
                    resultImage: imageUrl
...
export async function DELETE(req) {
  try {
    ...
    await prisma.groupPhotoCreation.delete({
      where: { id, userId: session.user.id }
    });
